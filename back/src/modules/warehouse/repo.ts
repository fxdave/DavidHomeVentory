...
export class WarehouseRepository {
  ...
  async create(data: {
    ...
    const parent = await this.__db.warehouseEntry.findFirst({
      where: {id: parentId},
    });
    ...
    const entry = await this.__db.warehouseEntry.create({
      data: {
        ...
        pathCache: PathCache.from(parent.pathCache)
    ...
    if (data.parentId) await this.__resetVariant(data.parentId);
  ...
  async update(data: {
    ...
    const oldParent: WarehouseEntry | null = await this.__getParentByItemId(
      data.id,
    );
    const newParent: WarehouseEntry | null = await this.__getItemOrNull(
      data.parentId,
    );
  ...
  private async __updateChildrenPathCache(
    id: string,
    oldPathCache: PathCache,
    ...
    for (const child of deepChildren) {
      await this.__db.warehouseEntry.update({
        ...
        data: {
          pathCache: PathCache.from(child.pathCache)
  ...
  private async __getDeepChildren(id: string) {
    const deepChildren: WarehouseEntry[] = [];
  ...
  private async __resetVariant(id: string) {
    ...
    else await this.__setVariant(id, WarehouseEntryVariant.Container);
  }
  ...
  private async __getParentByItemId(id: string) {
    ...
    return await this.__getItemOrNull(item.parentId);
  }
